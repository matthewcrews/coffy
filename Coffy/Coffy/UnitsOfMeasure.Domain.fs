namespace Coffy.UnitsOfMeasure

open Coffy
open Coffy.UnitsOfMeasure.Types
...
module Decision =
    ...
    let createInteger<[<Measure>] 'Measure> decisionName (lowerBound:float<'Measure>) (upperBound:float<'Measure>) =
        let d = Decision.createInteger decisionName (int lowerBound) (int upperBound)
        Decision<'Measure>.Value d
...
module Builders =
    ...
    type DecisionBuilder<[<Measure>] 'Measure> (decisionSetPrefix:string) =
        ...
        member this.Yield (decisionType:DecisionType<'Measure>) =
            ...
            | DecisionType.Boolean -> Coffy.Types.DecisionType.Boolean
            | DecisionType.Integer (lb, ub) -> Coffy.Types.DecisionType.Integer (int lb, int ub)
